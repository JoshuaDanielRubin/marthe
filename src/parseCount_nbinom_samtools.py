...
if True:
    ...
    name_bam=b.split("/")[-1]
    outputfilek = open (tfm+'/'+ options.resultso+"_karyo_"+name_bam+".txt" , 'w' ) 
    ...
    if(not os.path.exists(merged_count_file)):
        ...
        #countInfo = merge_files(options.tmpfolder, map_score_list, affectedrau, filtered_count_file, filter=False)
    ...
    command = 'Rscript'
    args = [filtered_gcContent, filtered_count_file]  #Filtered files
    cmd = [command, path_to_script] + args
    outputGC=subprocess.check_output(cmd, universal_newlines = True)    #This has corrected GC bias
    ...
    #filter the dictionary to have only the bins which have been used in the p estimation
    #gcBinsDict_filter = {key:value for key, value in gcbinsDict.items() if value!=-1}
    #gcKeys = list(gcBinsDict_filter.keys())

    for i in range(len(gcKeys)):
        #The p estimations and the keys are in the same order
        gcbinsDict[gcKeys[i]]["p_est"] = info_list[i][0]
        gcbinsDict[gcKeys[i]]["std"] = info_list[i][1]
    ...
    #for rau in range(len(conditions[8]["chrrange"])): 
        #print(conditions[8]["chrrange"][rau]["ecov"])
    print ("Evaluating every condition")


    for cd in tqdm(range(0, len(conditions))): #for each condition
    #for cd in tqdm(range(42, 43)):   #klinefelter
    #for cd in tqdm(range(1,2)):   #unaffectedmale
    #9 is cat eye male
        ...
        #for rau in range(10):
            ...
            #print(int(countInfo[rau]["corrected_read"]), r, p, llik)
            ...
            #print(rau, gcranges[rau]["chr"], str(int(countInfo[rau]["corrected_read"])), str(int(predicted)), round(p,4), round(r,3), round(llik,2))
            ...
            #print(gcranges[rau]["chr"], rau,  conditions[cd]["chrrange"][rau]["ecov"], conditions[-1]["chrrange"][rau]["ecov"], str(round(float(llik),2)),  str(round(float(llik_m),2)), str(int(countInfo[rau]["corrected_read"])))
            #print( gcranges[rau]["chr"], conditions[cd]["chrrange"][rau]["ecov"], conditions[42]["chrrange"][rau]["ecov"], str(int(countInfo[rau]["corrected_read"])), str(int(regionlambda)),  str(int(predicted)), str(int(predicted2)), str(round(float(llik),2)),  str(round(float(llik_m),2)))
            #print(rau, gcranges[rau]["chr"], str(countInfo[rau]["corrected_read"]), str(predicted), llik)
            #if llik_m != llik or llik_m!=llik_3:
                #print(gcranges[rau]["chr"], str(int(countInfo[rau]["corrected_read"])),  str(int(predicted)), str(int(predicted2)),str(int(predicted3)), str(round(float(llik),2)),  str(round(float(llik_m),2)),  str(round(float(llik_3),2)))
    ...
    if(True):    
        ...
        result_k = allpostllik[42]-sumpost    #Klinefelter
        result_j = allpostllik[46]-sumpost    #Jacobs
        
        result_t = allpostllik[43]-sumpost    #Turner
        ...
        results_post.write("Klinefelter")
