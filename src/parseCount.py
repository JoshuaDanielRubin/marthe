...
if True:
    ...
    for key in gcbinsDict:
        for row in range(len(gcranges)):
            ...
            if (float(gcranges[row]["gc"])>=float(key)) and (float(gcranges[row]["gc"])<float(key)+0.1):
                tmp_pos.append(row)
                tmp_read.append(str(int(countInfo[row]["count"])))
    ...
    df_toplot["GC_bins"] = list_gc
    df_toplot["Lambda"] = list_lambda
    df_toplot["Standard_deviations"] = list_std
    df_toplot["p_estimated"] = list_p
    df_toplot["r_fixed"] = list_r

    df_toplot.to_csv(results_path+"all_info.csv", index=False)
    ##########################################################
    ##########################################################
    ...
    if options.mappability == None:
        ...
        infile.close()
        #Ns_percen_l = calc_map_score("/home/databases/genomes/Homo_sapiens/hs37d5/mappability/snpable99/mappable_99.bed.gz")
        #Ns_percen_l = [1-score for score in Ns_percen_l]
    ...
    df_toplot = pd.DataFrame()
    observed_rc_l= []; predicted_rc_l = []; rau_l = []
    ...
    #for cd in tqdm(range(0,1)):
        ...
        #for rau in range(2878, 3030): 
            ...
            if gcbinsDict[gcround]["lambda"] == "NA":
                continue
            if( gcbinsDict[gcround]["lambda"] == -1 ): #GC bin was not defined by the Loess
                continue 
            ...
            if options.mappability == None:
                if Ns_percen_l[rau] == 0:
                    obs_reads = obs_reads
                elif Ns_percen_l[rau] < 0.85:  #I discard the rest
                    obs_reads = obs_reads/Ns_percen_l[rau]
                    predicted = predicted/Ns_percen_l[rau]
            ...
            #Save the observed number of reads and the predicted number of reads for the plots
            tmp_l.append(predicted)
            if len(observed_rc_l) < len(tmp_l):
                observed_rc_l.append(obs_reads)

            if len(rau_l) < len(tmp_l):
                rau_l.append(rau)
            ...
            llik = nbinom.logpmf(int(obs_reads), r, p)

            #print(rau, Ns_percen_l[rau], obs_reads,  predicted, llik)
    ...
    #After I have finished with all the conditions and regions, I create the df

    df_toplot["rau"] = rau_l
    df_toplot["Observed_rc"] = observed_rc_l

    for i in range(len(predicted_rc_l)):
        df_toplot[conditions[i]["namecond"]] = predicted_rc_l[i]

    df_toplot.to_csv(results_path+"conditions_rc.csv", index=False)
